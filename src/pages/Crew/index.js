...
const member = ["Douglas Hurley","Mark Shuttleworth","Victor Glover","Anousheh Ansari"]
const [index , setIndex] = useState(0);
  ...
  return (
    ...
    <article  className='lg:w-1/2 md:w-full lg:mr-80 lg:h-3/4'>
      <h1 className='CrewH1  text-white uppercase text-2xl lg:text-lg font-normal  lg:mr-12 sm:relative lg:top-0'><span className='text-gray-500 font-bold'>02   </span> Meet Your Crew</h1> 
      <img src={`${crew[index].images.png}`} alt={crew[index].name} className='crewMobimg sm:hidden'  ></img>
      <nav className='crewmobnav  flex  items-center justify-between  mt-6 lg:mt-16 sm:hidden' >
        ...
        {member.map((person,index)=>(<li><FontAwesomeIcon icon={faCircle} className="text-gray-600 hover:text-white hover:cursor-pointer" key={index+""} onClick={()=>{
                setIndex(index);}}/></li>))} </ul>
                </nav>
      <h2 className='commander lg:h-11 lg:p-2 lg:mt-18 text-white lg:text-3xl   uppercase sm:relative lg:top-11'>{crew[index].role}</h2>
      <h1 className='crewName lg:h-12 lg:p-2 text-white text-3xl sm:text-4xl h-12 uppercase lg:mt-12'>{crew[index].name}</h1>
  ...
  <li><FontAwesomeIcon icon={faCircle} className="text-gray-600 hover:text-white" /></li>*/}
     ...
     <article className='lg:mt-44 lg:w-3/5'>
      ...
      <img src={`${crew[index].images.png}`} alt={crew[index].name} className='crewImage'  ></img>
      </article>
      </div>
</section>
  )
